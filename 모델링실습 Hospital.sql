...
insert into `nurses` values ('N108101','송중기', '1992-10-07', 'M', 108, '간호사','010-7102-9210','jungi@bw.com');
...
insert into `charts` values ('C1031006', '1031006','D103101','P103102', '목 견인 치료');
...
#문제 1
select `doc_id`,`doc_name`,`doc_birth`,b.`dep_no`,b.`dep_name` from `doctors` as a
join `departments` as b on a.dep_no=b.dep_no;

#문제 2
select `nur_id`,`nur_name`,`nur_birth`,b.`dep_no`,b.`dep_name` from `nurses`as a
join `departments` as b on a.dep_no=b.dep_no;
...
#문제 5
select `treat_no`, `pat_name`, `doc_name`, `treat_contents`, `chart_contents`, `treat_datetime` from `treatments`as a
join `charts` using(treat_no)
join `doctors` as b on a.doc_id=b.doc_id
join `patients` as c on a.pat_id=c.pat_id
...
#문제 7 30세 이상~ 40세 미만 환자 조회
select * from
(select *, 
	year(now()) - concat('19', SubString(`pat_jumin`,1, 2)) as `age`
from 
	`patients`) as a 
    where a.`age`>=30 and a.`age`<40;
...
#문제 9 김태희 간호사가 맡고 있는 모든 환자의 이름을 구분자^으로 해서 조회
select
...
#문제 10 가장 많은 환자 수를 담당하는 간호사 ID, 간호사 이름, 담당환자 수 조회.
select a.`nur_id`, a.`nur_name`, 
count(b.`pat_id`) as '담당환자 수' 
from `nurses` as a
left join `patients` as b on a.nur_id=b.nur_id
group by a.`nur_id`, a.`nur_name`
order by `담당환자 수` desc limit 1;
