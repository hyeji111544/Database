...
select 
`regStdNo`, `regLecNo`,`regMidScore`,`regFinalScore`,`regMidScore`+`regFinalScore`as `totalGrade` ,
if ((`regMidScore`+`regFinalScore`) >=90, 'A', 
			if((`regMidScore`+`regFinalScore`) >=80, 'B',
			if((`regMidScore`+`regFinalScore`) >=70, 'C',
			if((`regMidScore`+`regFinalScore`) >=60, 'D', 'F')
            ))) as `regGrade` from  `register`;

select `stdNo`,`stdName`, `stdYear` from `Student` as a
left join `register` as b on a.stdno = b.regStdNo
where b.regStdNo is null;
